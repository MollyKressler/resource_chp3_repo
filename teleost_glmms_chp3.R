...
pacman::p_load(gbm,gbm.auto,tidyverse,sf,ggplot2,dismo)

## try first with grids=NULL. will need to predict to maps outside of package...unsure how.
## try with Gerreidae
# not using gbm.auto. I tried and we didn't get along. 

	#sw1<-gbm.auto(grids=NULL,samples=samples,expvar=c(2,13:20),resvar=c('SW_Species'),tc=c(9),lr=c(0.001,0.001),bf=0.9,fam1='bernoulli',fam2='gaussian',savedir='/Users/mollykressler/Documents/data_phd/resource_chp3',savegbm=TRUE,varint=TRUE)
...
expl<-joined_df[c(2,13:20)]
resp<-joined_df[c('Gerreidae')]
...
saveRDS(sw_species1,'SW_Species_brt_tc9_lr001_gaussian.RDS')
