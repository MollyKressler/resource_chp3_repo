...
#####################
### - July 2024: need to take covariates from former finished datsets and spatially join the data to the new BRT predictions. 
#####################

	pointdata <- read_csv('resource_chp3/standardised_meancentred_data_for_bayes_structural_EQ_modelling_optionC_sharkiness_fishiness_habitat_JUNE24.csv')%>%
		dplyr::select(-fish, -standard.fish, -standard.gerr, -gerr_preds)%>%
		rename(buffID = buffIDnum)

	hexdata <- st_as_sf(st_read('resource_chp3/data_for_bayes_structural_EQ_modelling_DF2_HEXAGONpredictions_fromPRuse_andBRTs_may24.shp'))%>%
	...
	fishhex <- st_as_sf(st_read('predictions_from_simplifiedBRTs_Gerreidae_and_sppRichness_june24.shp'), crs = 'WGS84')%>%
		dplyr::select(maxN_preds, geometry)

	fishbuff <- st_as_sf(st_read('sf_with_predictions_INTO_BUFFERS_fromBRTs_july2024.shp'), crs = 'WGS84')%>%
	...
	## - save: hexdata with new MaxN coV with spatial and as CSV, and pointdata as just a new csv. 
		...
		st_write(newhex, 'data_for_bayes_structural_EQ_modelling_DF2_HEXAGONpredictions_andBRTs_july24.shp', driver = 'ESRI Shapefile')
