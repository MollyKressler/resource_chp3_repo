...
## same plots, all navy 
	...
	pm.plot2<- ggplot()+
		...
		ylab('Large Sharks (standardised)')+xlab('Distance from Shore (standardised)')+
		geom_line(data=pm..predicts.dist2shore,aes(x=x,y=predicted),col='#3a6c74',size=.6,linetype=1)+
		geom_ribbon(data=pm..predicts.dist2shore,aes(x=x,ymin=conf.low,ymax=conf.high),alpha=0.3, fill='#3a6c74')+
		theme_bw()
